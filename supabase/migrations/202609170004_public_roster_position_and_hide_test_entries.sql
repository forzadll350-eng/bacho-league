-- Show the personnel category needed for roster checks and hide the two
-- organizer-identified test registrations without deleting their audit trail.

update public.registrations
set
  public_roster_consent = false,
  public_roster_consented_at = null
where
  lower(regexp_replace(btrim(full_name), '\s+', ' ', 'g')) in ('test tes2', 'test test2')
  or btrim(full_name) = 'นายอาลีฟ ดอเล๊าะ';
...
begin
  ...
  if p_privacy_acknowledged is distinct from true
     or p_privacy_notice_version is distinct from 'privacy-notice-2026-09-17-v3' then

-- Record rules/privacy acknowledgements at registration time and expose a
-- deliberately narrow public roster only for athletes who opt in.
...
alter table public.registrations
  ...
  add column if not exists public_roster_consent boolean not null default false,
  add column if not exists public_roster_consented_at timestamptz;

comment on column public.registrations.rules_version is
  'Version of the futsal rules accepted during registration; null for volleyball and legacy rows';
comment on column public.registrations.privacy_notice_version is
  'Version of the privacy notice acknowledged during registration';
comment on column public.registrations.public_roster_consent is
  'Optional consent to appear in the narrow public roster view';

drop function if exists public.submit_player_registration(
...
begin
  ...
  if p_privacy_acknowledged is distinct from true
     or p_privacy_notice_version is distinct from 'privacy-notice-2026-09-17-v1' then
  ...
  insert into public.registrations (
    ...
    statement_timestamp(),
    coalesce(p_public_roster_consent, false),
    case when coalesce(p_public_roster_consent, false) then statement_timestamp() else null end
  )
...
drop view if exists public.public_player_roster;
create view public.public_player_roster
...
as
  select
    ...
    team_id,
    full_name,
    case when sport = 'football' then jersey_number else null end as jersey_number
  from public.registrations
  where public_roster_consent is true;

revoke all on public.public_player_roster from public;
grant select on public.public_player_roster to anon, authenticated;
